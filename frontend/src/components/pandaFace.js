...
function pandaFace({handsUp, inputNum, charNum, seeking, cursorPosition}){
      ...
      if ( !isNil(inputNum)){
            ...
            const position1 = {x: x0 + r * Math.sin(theta.left + myAngle),y: y0 + r * Math.cos(theta.left + myAngle)};
  
            const position2 = [x0 + r * Math.sin(theta.right + myAngle),];
            position2[1] = y0 + r * Math.cos(theta.right + myAngle);
          
            positionIrisRight = {transform: "translate(" +  position1.x + "px," + position1.y + "px)"};
            positionIrisRight.transform += " rotate(20deg)";
            positionIrisLeft = {transform: "translate(" +  position2[0] + "px," + position2[1] + "px)"};
            positionIrisLeft.transform += " rotate(-20deg)";
      }

      return (
            <div class={"panda" + (handsUp ? " handsUp" : "") + (seeking ? " seeking" : "")}>
                                          <div class="ears">
                                                <div class="leftEar" />
                                                <div class="rightEar" />
                                          </div>
                                          <div class="head">
                                                <div class="eyes">
                                                      <div class="leftEye">
                                                            <div class="irisEye" style={positionIrisLeft}/>
                                                      </div>
                                                      <div class="rightEye">
                                                            <div class="irisEye" style={positionIrisRight}/>
                                                      </div>
                                                </div>
                                                <div class="face">
                                                      <div class="nose">
